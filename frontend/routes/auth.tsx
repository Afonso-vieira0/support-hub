...
function AuthPage() {
  ...
  const onGoogle = async () => {
    ...
    if (result.error) {
      setLoading(false);
      return toast.error("Falha ao iniciar sessão com Google. Configure o provider no painel.");
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-background via-background to-accent/30 p-4">
      <div className="w-full max-w-md">
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-primary-glow text-primary-foreground shadow-lg">
            <HeadphonesIcon className="h-6 w-6" />
          </div>
          <h1 className="text-2xl font-bold">SupportHub</h1>
          <p className="text-sm text-muted-foreground">Plataforma de suporte técnico</p>
        </div>
          ...
          {forgot ? (
            <>
              <CardHeader>
                ...
                <CardDescription>Enviaremos um email com o link de recuperação.</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                ...
                <TabsContent value="signup" className="mt-2 space-y-4">
                  <form onSubmit={onSignup} className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="su-password">Palavra-passe</Label>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          O primeiro utilizador registado será automaticamente Super Admin.
        </p>
      </div>
    </div>
  );
